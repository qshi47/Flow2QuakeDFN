...
function If_FaultPatch_on_Reservoir(Criterion_for_Fault_within_Reservoir::Function, SingleFaultCenter, PorePressureChange, timeidx, idx_nearest_cuboid )
   ...
   if Criterion_for_Fault_within_Reservoir(SingleFaultCenter) == true
        ...
        porePressureChange_of_patch = PorePressureChange[timeidx, idx_nearest_cuboid] 
   end
...
function Calculate_Nearest_CubeIdx_to_Fault(Cuboids_Center, SingleFaultCenter)
    ...
    idx_nearest_cuboid =  argmin(distances)
...
function Rotate_Tensor_to_Stress_on_Fault(RotationMat_FromFault, sigEff_all, FaultDipAngle, FaultStrikeAngle, FaultLLRR, SSorRN)    
...
function main(Criterion_for_Fault_within_Reservoir::Function, Input_Fault_File, Input_Cuboids_File, Input_PorePressure_File, Output_ExternalStress_File)
    ...
    for (TimeIdx, Time) in tqdm( enumerate(ExternalStress_TimeArray), unit = " timestep" )
        for i =1:FaultCount
            ...
            Rotate_Tensor_to_Stress_on_Fault(RotationMat_FromFault_All[i,:,:], SigEff_all,  
