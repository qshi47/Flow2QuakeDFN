...
PressureFile = "CuboidCoupling/Input_PorePressure.txt"
...
ExternalStress_TimeArray = Float64.(readdlm(PressureFile, ',' )[1,2:end])

# load pressure data
PorePressureChange = Float64.(readdlm(PressureFile, ',')[2:end,2:end]')

# interpolate pore pressure to rupture time array
PorePressure_Interpolated = zeros(length(HistoryTime), Cuboids_Count)
@info "Interpolating pore pressure for $(length(HistoryTime)) time steps"

for idx_Cuboid in tqdm( 1:Cuboids_Count, unit = "cuboid" )
    xs = ExternalStress_TimeArray
    ys = PorePressureChange[:, idx_Cuboid]
    interp_linear = linear_interpolation(xs, ys)
    PorePressure_Interpolated[:, idx_Cuboid] = interp_linear(HistoryTime)
end
...
function write_cuboid_series_vtu(
    ...
    nt, nc_p = size(pressure)
    @assert nc_p == nc                       "pressure must be (nt, nc)"
    ...
    @inbounds for it in tqdm( 1:nt, unit = "time step" )
        ...
        vtk_grid(fname, points, cells; compress=compress) do vtk
            vtk["pressure", VTKCellData()] = Float64.(view(pressure, it, :))
...
write_cuboid_series_vtu(Cuboids_Center, Cuboids_Length./2, PorePressure_Interpolated, HistoryTime/Year_in_Second; outdir="VTK/CuboidsPressure_vtu")

# write_cuboid_series_vtu(Cuboids_Center, Cuboids_Length./2, PorePressureChange, ExternalStress_TimeArray, outdir="VTK/CuboidsPressure_vtu")