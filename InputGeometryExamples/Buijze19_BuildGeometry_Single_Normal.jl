...
function BuildBulk()
    ...
    Thickness = load(LoadingBuijzeGeomery, "Thickness") 
    Offset = load(LoadingBuijzeGeomery, "Offset")
    Cube_Size_2D = load(LoadingBuijzeGeomery, "Cube_Size_2D")

    FaultCenterX = load(LoadingBuijzeGeomery, "FaultCenterX")
    FaultCenterY = load(LoadingBuijzeGeomery, "FaultCenterY")
    FaultCenterZ = load(LoadingBuijzeGeomery, "FaultCenterZ")
    LengthDip = load(LoadingBuijzeGeomery, "LengthDip")
    LengthStrike = load(LoadingBuijzeGeomery, "LengthStrike")

    DipAngle = 180.0 - Phi # deg
    println("Dip Angle is ", DipAngle)
    
    MyPossionRatio = 0.15
    MyYoungModulus = 15e9
    MyShearModulus = MyYoungModulus/(2*MyPossionRatio+2)

    SwitchSSRN = 2 # The Fault in Buijze19 3D is a 3D (Compressing) Normal fault
