...
function FaultPlot_3D_v2!(fig, ax, FaultNormalStress, sm, FaultCenter, 
    ...
    # MaxValue=maximum(FaultNormalStress)
    # MinValue=minimum(FaultNormalStress)
    ...
    for FaultIdx in eachindex(FaultLengthStrike)
        ...
        p3c = PyObject(art3d.Poly3DCollection(verts2, linewidths=1, alpha=0.8))
        ...
        PlotValue=(FaultNormalStress[FaultIdx] - MinValue) / (MaxValue - MinValue)
        
        face_color = [cm(PlotValue)[1], cm(PlotValue)[2],cm(PlotValue)[3],1]
        # face_color = [0, 0, 1]

        # edge_color = [0, 0, 0] # black
        edge_color = [128/256, 128/256, 128/256]
